...
| [`lua/corpus_coagulant/shared/corpus_coagulant_items.lua`](lua/corpus_coagulant/shared/corpus_coagulant_items.lua) | shared | Ítem `corpus_coagulant_bandage` contra Cargo (soft-dep, en `Corpus.OnReady`; el set de 4 llega con el slice 2) + `coagulant_bandage` (debug, server). **Shared obligatorio: Cargo no sincroniza defs por net** — registrarlas solo en server = ítem invisible en la UI (pagado el 2026-07-13) |
