...
**Estado actual: BLOCK 3 CERRADO (ronda 7, 2026-07-20).** El diseño está en [`docs/Coagulant_Architecture.md`](docs/Coagulant_Architecture.md) (ratificado por el autor el 2026-07-13; números de balance tunables). Los **4 slices están verificados en juego** (rondas 1-7; la 7 cerró la UI, el sway retuneado y el modo degradado sin Cargo) y los **fixes post-cierre están confirmados** (mini-ronda 8: 2/2, check N1 ✓). El **tramo de zonas `torso` → `chest` & `stomach` está COMPLETO** (COA-8/COA-7): ratificado, bajado a código y verificado en juego el 2026-07-21 (ronda O: 6/6) — las zonas clínicas ya son **7**. Pendientes: **dos decisiones de diseño abiertas** del autor (toggle del paperdoll; si la tecla del menú también cierra) y **re-correr el check 4 de COA-37** en juego — ver [`docs/coagulant_estado.md`](docs/coagulant_estado.md). **COA-37 (2026-07-29, enmienda ratificada y verificada 4/5):** la venda aplica a toda herida **abierta**, no solo a las que sangran — antes la `contusion` (mult 0.0) era invendable y, como el medkit solo borra lo ya `treated`, **incurable**. Es la salida (1) de las tres que ACE3 le da al trauma cerrado; dolor con analgésico y fractura con férula siguen diferidos (arquitectura §1, §7). Al elegir entre dos heridas abiertas **manda el sangrado**, no la severidad (corregido el mismo día tras la pasada: un moretón medio le ganaba a un balazo leve). **COA-28 — No implementes nada que la arquitectura no especifique** — cambios de diseño se discuten con el autor y se anotan primero en el doc.
