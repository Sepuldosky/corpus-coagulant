...
**Estado actual: BLOCK 3 EN BAJADA (slice 1 de 4).** El diseño está en [`docs/Coagulant_Architecture.md`](docs/Coagulant_Architecture.md) (ratificado por el autor el 2026-07-13 al ordenar la bajada; números de balance tunables). El slice 1 (sangre + heridas + sangrado) está en código; tratamiento con tiempo/ítems (slice 2), debuffs (slice 3) y UI (slice 4) siguen la secuencia de §15 de la arquitectura. **No implementes nada que la arquitectura no especifique** — cambios de diseño se discuten con el autor y se anotan primero en el doc.
...
| [`lua/corpus_coagulant/server/corpus_coagulant_items.lua`](lua/corpus_coagulant/server/corpus_coagulant_items.lua) | server | Ítem `corpus_coagulant_bandage` contra Cargo (soft-dep, en `Corpus.OnReady`; el set completo de 4 llega con el slice 2) + `coagulant_bandage` (debug) |
