...
- **Dolor (COA-52), desde el 2026-08-25** — estado **por zona derivado** (producción
  ...
  sobre el percibido — la 2.ª dosis se desgatilla sola). **Sin pasada en juego.**
- **Verificación offline:** sintaxis (`dev/glua_check.py`, 13 archivos) + harness
  ...
  checks de la partición incluidos + selftest en ambos realms): **275 checks, 0
  fallos, ALL GREEN**; selftest **221 OK server / 161 client**. Verificación en
  negativo del tramo del dolor:
  [`../../dev/sabotaje_coagulant_dolor.py`](../../dev/sabotaje_coagulant_dolor.py).
...
- **EL DOLOR ENTERO, y es lo primero de la lista** (2026-08-25): planilla en
  `dev/checks/coagulant-dolor-r1.html`, publicada en
  [este artefacto](https://claude.ai/code/artifact/f9e534e3-2843-453c-88bf-e786e6778cd1). Qué mirar, en un gesto solo: te disparan →
  `coagulant_status` imprime el dolor por zona y el global **en sus dos formas**
  (crudo y percibido) → el menú médico dice cuánto duele esa zona → te tomas las
  Painkillers → el percibido baja y el botón se apaga solo → a los ~3 min vuelve a
  subir. ⚠ **Los dos votos que el autor puede vetar barato** están en la planilla y
  se marcan como delegados: que el analgésico exista (voto B del §3.1 del prompt) y
  que la silueta NO cambie de insumo (§3.2).
